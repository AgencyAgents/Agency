...
|---|---|---|---|
| RoomStore and appendMessage broadcast | Yes, with tests | No: zero non-test instantiations, no room RPC, no room tool | Team lifecycle with room IDs per turn |
...
| Room protocol text and appendRoomProtocol | Yes, baked into role tables | Partial: present in prompt strings, never shown to a peer at runtime | Playbook as the shared cached prefix |
...
| Team isolation (per-room sessions, worktrees, mailboxes) | Partial: SessionScope isolates tools and state per session | No: handles, worktrees, mailboxes, and cost maps are daemon-global; SessionScope roomId is never set | Disjoint path scopes plus lead-owned integration |
...
| Boulder checklist store | Yes, with tests | No runtime consumer | Progress file as the board substrate |
