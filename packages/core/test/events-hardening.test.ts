...
describe("EventBus hardening", () => {
  test("wildcard * matches any event", () => {
    ...
    const seen: string[] = [];
    bus.on("*", (p) => seen.push((p as { event: string }).event));
  ...
  test("error isolation: throwing listener does not break emit loop", () => {
    const bus = new EventBus();
    const calls: string[] = [];
    bus.on("log.entry", () => { throw new Error("boom"); });
    bus.on("log.entry", () => calls.push("second"));
  ...
  test("async listener throwing does not break loop (emit)", () => {
    const bus = new EventBus();
    const calls: string[] = [];
    bus.on("log.entry", async () => { throw new Error("async boom"); });
    bus.on("log.entry", () => calls.push("ok"));
    bus.emit("log.entry", { level: "info", message: "hi" });
  ...
  test("emitAsync awaits listeners and isolates errors", async () => {
    ...
    bus.on("tool.execute.before", () => { throw new Error("fail"); });
    bus.on("tool.execute.before", async () => order.push("c"));
  ...
  test("emitCollect captures throwing listener errors for short-circuit", async () => {
    const bus = new EventBus();
    bus.on("tool.execute.before", () => { throw new Error("short-circuit"); });
  ...
  test("wildcard pattern with prefix * suffix", () => {
    const bus = new EventBus();
    const hits: string[] = [];
    bus.on("*.edited", (p) => hits.push((p as { path: string }).path));
