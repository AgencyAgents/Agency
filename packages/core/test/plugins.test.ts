...
class FakeRegistry {
  ...
  register(spec: { name: string }) { if (this.m.has(spec.name)) throw new Error(`tool "${spec.name}" is already registered`); this.m.set(spec.name, spec); }
  has(name: string) { return this.m.has(name); }
  list() { return [...this.m.values()] as never[]; }
}
...
describe("plugin loader", () => {
  test("discovers and loads project plugin", async () => {
    ...
    writeFileSync(pluginPath, `export const hooks = { "tool.execute.before": async () => {} }; export const tools = [{ name: "mytool", description: "hi", inputSchema: { type: "object", properties: {} }, riskTier: "safe", handler: async () => ({ content: "ok" }) }];`);
  ...
  test("plugin tool namespacing: registry uses pluginId_toolName", async () => {
    ...
    mkdirSync(join(ws, ".agency", "plugins"), { recursive: true });
    writeFileSync(join(ws, ".agency", "plugins", "myplugin.js"), `export const tools = [{ name: "greet", description: "g", inputSchema: { type: "object" }, riskTier: "safe", handler: async () => ({ content: "hi" }) }];`);
    const bus = new EventBus();
    const registry = new FakeRegistry();
  ...
  test("one bad plugin does not break others (error isolation)", async () => {
    ...
    writeFileSync(join(ws, ".agency", "plugins", "good.js"), `export const hooks = { "session.created": async () => {} };`);
    const bus = new EventBus();
  ...
  test("hooks firing: plugin hook receives bus event", async () => {
    ...
    mkdirSync(join(ws, ".agency", "plugins"), { recursive: true });
    writeFileSync(join(ws, ".agency", "plugins", "hooktest.js"), `export const hooks = { "tool.execute.before": async (payload, ctx) => { globalThis.__hookCalled = (globalThis.__hookCalled || 0) + 1; }, "file.edited": async (payload) => { globalThis.__fileEdited = payload.path; } };`);
    const bus = new EventBus();
  ...
  test("tool.execute.before hook can short-circuit tool execution via error isolation + emitCollect", async () => {
    const bus = new EventBus();
    bus.on("tool.execute.before", () => { throw new Error("deny this tool"); });
    const fakeAdapter: ProviderAdapter = {
      stream: async function* () {
        yield { type: "tool_call_start", id: "c1", name: "bash" } as never;
        yield { type: "tool_call_delta", id: "c1", inputJsonDelta: JSON.stringify({ command: "echo hi" }) } as never;
        yield { type: "tool_call_end", id: "c1" } as never;
        yield { type: "message_stop", usage: { inputTokens: 1, outputTokens: 1 }, stopReason: "tool_use" } as never;
      },
    };
    const fakeScheduler = { schedule: (fn: () => Promise<never>) => fn() } as unknown as import("@agency/providers").Scheduler;
    ...
    const userMsg = result.messages.find((m) => m.role === "user" && m.content.some((b) => b.type === "tool_result"));
    const toolResult = userMsg?.content.find((b) => b.type === "tool_result") as { content: string; isError: boolean } | undefined;
  ...
  test("wildcard hook receives multiple event types", async () => {
    ...
    mkdirSync(join(ws, ".agency", "plugins"), { recursive: true });
    writeFileSync(join(ws, ".agency", "plugins", "wild.js"), `export const hooks = { "tool.*": async (payload) => { globalThis.__wildCount = (globalThis.__wildCount || 0) + 1; } };`);
    const bus = new EventBus();
  ...
  test("plugin hook throwing does not crash bus (error isolation)", async () => {
    ...
    bus.on("session.created", () => { throw new Error("plugin boom"); });
    bus.on("session.created", () => { secondCalled = true; });
